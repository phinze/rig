...
func renderRigInstructions(basedir string, m manifest) string {
	...
	b.WriteString("  Park means waiting on someone else; down means finished, nothing to wait for.\n")
	// A review rig's work is the review, not a PR of its own, so the two
	// verdicts split it: an approval leaves nothing to wait on, while a changes
	// request waits on the author's push.
	if m.isReview() {
		b.WriteString("  Once your review is posted, say the rig is ready to tear down if it\n")
		b.WriteString("  approved, or ready to park if it requested changes, and stop.\n\n")
	} else {
		b.WriteString("  When the work is up for review, say the rig is ready to park and stop.\n")
		b.WriteString("  When it has merged or finished with nothing outstanding, say it is ready\n")
		b.WriteString("  to tear down.\n\n")
	}
...
func renderProjectRigInstructions(basedir string, m manifest) string {
	...
	b.WriteString("- Reconcile the path to completion: what is done and ready to close or merge;\n")
	b.WriteString("  what is stale or inconsistent in Linear; what work or dependencies are\n")
	b.WriteString("  missing; and which task rigs are blocked, abandoned, waiting on review, or\n")
	b.WriteString("  ready to resume. Health fields and project updates are context, not work to\n")
	b.WriteString("  maintain by default.\n")
	b.WriteString("- Work as the user's project collaborator, not an independent executor. Propose\n")
	b.WriteString("  the next concrete actions needed to finish the project, then wait for\n")
	b.WriteString("  confirmation before merging, dispatching, switching, parking, creating an\n")
	b.WriteString("  issue, or otherwise changing another rig or shared project state. Read-only\n")
	b.WriteString("  investigation does not need confirmation. Use `rig switch ISSUE` when a task\n")
	b.WriteString("  needs direct human attention.\n")
	b.WriteString("- To start a task rig for an issue on the user's behalf, run `rig up ISSUE\n")
	b.WriteString("  --repo owner/repo --context '<what its agent should know>'`. It comes up in\n")
	b.WriteString("  the background, and the context lands in that rig's KICKOFF.md, which its\n")
	b.WriteString("  agent is told to read alongside the ticket. Put the color the ticket lacks\n")
	b.WriteString("  there: the project's current shape, sibling work to build on or avoid, what\n")
	b.WriteString("  you already learned. Piped stdin works for a longer brief.\n")
	...
	b.WriteString("  to run its address-pr-review workflow. Dispatch refuses a live agent, and\n")
	b.WriteString("  `rig up` on an existing rig drops its --context, so pick by whether the rig\n")
	b.WriteString("  already exists.\n")
	b.WriteString("- This overview is torn down explicitly by the human (`rig down` from another\n")
	b.WriteString("  session, or the radar's ctrl+x leave menu) when the project no longer needs\n")
	b.WriteString("  coordination. Ordinary sweep will not collect it, and `rig down` refuses\n")
	b.WriteString("  from inside this session without a terminal. Say when it looks done.\n\n")
